...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const createdUser = await User.findById(user._id).select(
    "-password -refreshToken -emailVerficationToken -emailVerifcationExpiry",
  );
  if (!createdUser) {
...
const login = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordValid) {
    throw new ApiError(400, "Envalid credintials");
  }
...
const verifyEmail = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    throw new ApiError(400, "Email verification token is invalid or expire");
  }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const incomingRefreshToken =
    req.cookies.refreshToken || req.cookies.accessToken;

  if (!incomingRefreshToken) {
    throw new ApiError(401, "Unauthrizsed access");
  }
...
const forgotPasswordRequest = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne({ email });
  if (!user) {
    throw new ApiError(404, "Email is user does not exist");
  }
